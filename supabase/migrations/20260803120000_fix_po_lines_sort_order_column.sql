-- ============================================================
-- Migration: fix save_po_with_lines — sort_order column does not exist
-- Applied: 2026-08-03
-- Purpose:
--   save_po_with_lines (20260710030000_po_transactional_save.sql) inserts
--   into purchase_order_lines with a sort_order column that was never
--   added to that table (unlike order_rows/bom_lines, which do have one).
--   Every PO save with lines has been failing with:
--     "column \"sort_order\" of relation \"purchase_order_lines\" does not exist"
--   Fix: stop referencing the nonexistent column. Line order was never
--   persisted for purchase_order_lines before the atomic-RPC migration
--   either (the pre-RPC insert code didn't send it), so this restores
--   actual prior behavior rather than adding a new column.
-- ============================================================
